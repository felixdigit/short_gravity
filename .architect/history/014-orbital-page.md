...
---
MISSION:
Build the /orbital constellation health dashboard — TLE freshness, space weather, drag trends, source divergence, and conjunction events.

DIRECTIVES:

## 1. Create the /orbital page

Create `src/app/orbital/page.tsx`. This page wraps in `TerminalDataProvider` to access all satellite and space data.
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {SATELLITES_ORDERED.map(({ id, name, type }) => {
              ...
              // Status: green < 12h, yellow < 24h, red > 24h, gray = no data
              const status = hoursOld == null ? 'inactive'
                : hoursOld < 12 ? 'online'
                : hoursOld < 24 ? 'warning'
                : 'error'
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {divergenceData.map((row, i) => {
                    const delta = Math.abs((row.celestrakBstar ?? 0) - (row.spacetrackBstar ?? 0))
                    const flagged = delta > 0.0001
                    return (
                      <tr key={i} className="border-b border-white/[0.03]">
                        <td className="text-white/50 py-1.5 pr-4">{row.noradId ?? row.name ?? '—'}</td>
                        <td className="text-right text-white/60 tabular-nums py-1.5 px-3">
                          {row.celestrakBstar?.toExponential(2) ?? '—'}
                        </td>
                        <td className="text-right text-white/60 tabular-nums py-1.5 px-3">
                          {row.spacetrackBstar?.toExponential(2) ?? '—'}
                        </td>
                        <td className="text-right text-white/60 tabular-nums py-1.5 px-3">
                          {delta.toExponential(2)}
                        </td>
                        <td className={`text-right py-1.5 pl-3 ${flagged ? 'text-amber-400' : 'text-white/20'}`}>
                          {flagged ? 'DIVERGED' : 'OK'}
                        </td>
                      </tr>
                    )
...
## 2. IMPORTANT: Adapt field names to actual types

The code above uses field names that may not match the actual TypeScript interfaces. Before running tsc:

- Check `SpaceWeatherDay` type from `src/lib/hooks/useSpaceWeather.ts` — adapt field names for the space weather table (might be `f10_7`, `kp_index`, `ap_index`, `sunspot_number`, etc.).
- Check `DivergenceData` type from `src/lib/hooks/useDivergence.ts` — adapt field names for divergence table (might be `celestrak_bstar`, `spacetrack_bstar`, `norad_id`, etc.).
- Check `ConjunctionEvent` type from `src/lib/hooks/useConjunctions.ts` — field names were fixed in mandate 011.
- Check `DragHistoryData` type from `src/lib/providers/TerminalDataProvider.tsx` — should match.

Use optional chaining (`?.`) and nullish coalescing (`??`) throughout for safety. The goal is type-safe code that compiles, not perfect rendering on first load.
