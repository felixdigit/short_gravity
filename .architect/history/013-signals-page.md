...
---
MISSION:
Build the /signals intelligence feed page — a full-page signal browser with severity/type filtering.

DIRECTIVES:

## 1. Verify the useSignals hook exists

Check `src/lib/hooks/useSignals.ts`. It should exist and export a `useSignals()` function.
...
import type { Signal } from '@shortgravity/ui'
...
## 2. Check the Signal type

Read `@shortgravity/ui` types (in `packages/ui/src/types.ts` or wherever `Signal` is exported). The Signal type should include: `id`, `signal_type`, `severity`, `category`, `title`, `description`, `source_refs`, `metrics`, `confidence_score`, `fingerprint`, `detected_at`, `expires_at`, `created_at`.

If the Signal type doesn't exist in @shortgravity/ui, define it locally in the page or in a types file.

## 3. Create the /signals page
...
import { Panel, Text, Label, Muted } from '@shortgravity/ui'
...
Adapt the code if needed to match the actual Signal type shape. The Signal type from @shortgravity/ui may use different field names. Check the actual type definition and adjust property access accordingly. The important fields are:
- `severity` (string)
- `signal_type` (string)
- `title` (string)
...
- `metrics` (object with observed_value, baseline_value, z_score)
- `fingerprint` (string)
- `id` (number)

If SignalCard from @shortgravity/ui is a better fit than custom rendering, use it instead. But verify it accepts the Signal shape from the API.
