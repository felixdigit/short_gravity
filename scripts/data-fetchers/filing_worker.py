...
# 8-K items that warrant critical severity
CRITICAL_8K_ITEMS = {"1.01", "1.02", "2.01", "2.02", "2.03", "3.02", "5.02"}
...
def create_filing_signal(filing: Dict, summary: str) -> bool:
    """Create a signal for a newly detected filing."""
    ...
    # Determine severity
    severity = FORM_SEVERITY.get(form, "low")
    if form in ("8-K", "8-K/A") and items:
        item_set = {i.strip() for i in items.split(",")}
        if item_set & CRITICAL_8K_ITEMS:
            severity = "critical"

    # Skip low-signal forms (4, SC 13G, etc.)
    if severity == "low":
        return False

    # Build signal
    item_descs = get_item_descriptions(items)
    title_suffix = f" ({', '.join(item_descs)})" if item_descs else ""
    fp = hashlib.sha256(f"filing_drop|{accession}".encode()).hexdigest()[:32]

    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    expires = (datetime.now(timezone.utc) + timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%SZ")

    signal = {
        "signal_type": "filing_drop",
        "severity": severity,
        "title": f"New {form} filed{title_suffix}",
        "description": summary[:500] if summary else f"AST SpaceMobile filed a {form} on {filing_date}.",
    ...
    try:
        ...
        log(f"  SIGNAL: {severity.upper()} — New {form} filed")
        return True
    except Exception as e:
        log(f"  Signal creation failed: {e}")
        return False
