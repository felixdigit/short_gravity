#!/usr/bin/env python3
"""
FCC ICFS ServiceNow Worker — Scrapes the new FCC ICFS portal for ASTS-relevant satellite filings.
...
def discover_related_filings(page: Any, anchor: str) -> Set[str]:
    ...
    try:
        ...
        try:
            ...
            log(f"  networkidle timeout on Related Filings tab (non-fatal): {e}")
            ...
            for btn in page_btns:
                ...
                if "setPageNum" in ng_click:
                    try:
                        ...
                        log(f"  Pagination click error on page {next_page}: {e}")
                        break
                    time.sleep(2)
                    try:
                        page.wait_for_load_state("networkidle", timeout=10000)
                    except Exception as e:
                        log(f"  networkidle timeout on page {next_page} (non-fatal): {e}")
                    text = page.evaluate("() => document.body?.innerText || ''")
        ...
        try:
            page.screenshot(path=f"/tmp/debug_icfs_discovery_{anchor}_{int(time.time())}.png")
            log(f"  DEBUG: Screenshot saved for discovery failure analysis")
        except Exception:
            pass
...
def scrape_filing_detail(page: Any, file_number: str) -> Optional[Dict]:
    ...
    try:
        ...
        # Validate critical fields — skip filings with empty extraction
        parsed_file_number = fields.get("file_number", "")
        parsed_filer_name = fields.get("filer_name", "")
        if not parsed_file_number and not parsed_filer_name:
            log(f"    WARNING: Critical fields empty after parsing (file_number={parsed_file_number!r}, filer_name={parsed_filer_name!r}). Skipping to avoid garbage data.")
            return None
        ...
        try:
            page.screenshot(path=f"/tmp/debug_icfs_detail_{file_number}_{int(time.time())}.png")
            log(f"    DEBUG: Screenshot saved for detail failure analysis")
        except Exception:
            pass
...
def run(backfill: bool = False, dry_run: bool = False):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            log(f"\n  Parse results: {parse_success} success, {parse_fail} failed ({parse_fail}/{parse_success + parse_fail} failure rate)")
        finally:
            browser.close()
