    ...
    for attempt in range(3):
        try:
            ...
            try:
                ...
                pass
            if e.code == 429 or "rate" in error_body.lower():
                wait = 2 ** (attempt + 1)
                log(f"  Rate limited, retrying in {wait}s (attempt {attempt + 1}/3)")
                time.sleep(wait)
            elif attempt == 2:
                log(f"  ERROR: Claude API failed after 3 attempts: HTTPError {e.code} - {error_body}")
                raise
            else:
                wait = 2 ** (attempt + 1)
                log(f"  Claude API HTTPError {e.code}, retrying in {wait}s (attempt {attempt + 1}/3)")
                time.sleep(wait)
        except urllib.error.URLError as e:
            if attempt == 2:
                log(f"  ERROR: Claude API failed after 3 attempts: URLError {e.reason}")
                raise
            wait = 2 ** (attempt + 1)
            log(f"  Claude API URLError ({e.reason}), retrying in {wait}s (attempt {attempt + 1}/3)")
            time.sleep(wait)
        except Exception as e:
            if attempt == 2:
                log(f"  ERROR: Claude API failed after 3 attempts: {type(e).__name__}: {e}")
                raise
            wait = 2 ** (attempt + 1)
            log(f"  Claude API {type(e).__name__}, retrying in {wait}s (attempt {attempt + 1}/3)")
            time.sleep(wait)
    # Should not reach here, but just in case
    raise RuntimeError("haiku_classify: all retry attempts exhausted")
...
# Module-level counters for run summary
_stats = {"duplicates": 0, "store_errors": 0}
...
def store_signal(signal: Dict, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        log(f"  Error storing signal (HTTPError {e.code}): {e}")
        _stats["store_errors"] += 1
        return False
    except urllib.error.URLError as e:
        log(f"  Error storing signal (URLError): {e.reason}")
        _stats["store_errors"] += 1
        return False
    except Exception as e:
        log(f"  Error storing signal ({type(e).__name__}): {e}")
        _stats["store_errors"] += 1
        return False
...
Write a 2-sentence intelligence briefing about this sentiment shift. Be specific about what's driving it. No hedging.""")
                ...
                log(f"  Haiku error ({type(e).__name__}): {e}")
...
Write a 2-sentence intelligence briefing about what these language changes signal for ASTS. Be specific and actionable.""")
                ...
                log(f"  Haiku synthesis error ({type(e).__name__}): {e}")
        ...
        log(f"  Error in earnings analysis (HTTPError {e.code}): {e}")
    except urllib.error.URLError as e:
        log(f"  Error in earnings analysis (URLError): {e.reason}")
    except Exception as e:
        log(f"  Error in earnings analysis ({type(e).__name__}): {e}")
...
def detect_competitor_milestones(dry_run: bool = False) -> List[Dict]:
    ...
    try:
        ...
        log(f"  Error checking FCC grants (HTTPError {e.code}): {e}")
    except urllib.error.URLError as e:
        log(f"  Error checking FCC grants (URLError): {e.reason}")
    except Exception as e:
        log(f"  Error checking FCC grants ({type(e).__name__}): {e}")
    ...
    try:
        ...
        log(f"  Error checking patent grants (HTTPError {e.code}): {e}")
    except urllib.error.URLError as e:
        log(f"  Error checking patent grants (URLError): {e.reason}")
    except Exception as e:
        log(f"  Error checking patent grants ({type(e).__name__}): {e}")
...
def run_scanner():
    ...
    for name, detector in run_list.items():
        try:
            ...
            log(f"  ERROR in {name} detector (HTTPError {e.code}): {e}")
            errors += 1
        except urllib.error.URLError as e:
            log(f"  ERROR in {name} detector (URLError): {e.reason}")
            errors += 1
        except Exception as e:
            log(f"  ERROR in {name} detector ({type(e).__name__}): {e}")
            errors += 1
    ...
    if not args.dry_run:
        try:
            ...
            supabase_request("DELETE", f"signals?expires_at=lt.{now_iso}")
            log(f"Cleaned up expired signals (expires_at < {now_iso})")
        except urllib.error.HTTPError as e:
            # expires_at column may not exist yet
            if e.code == 400:
                log("  Skipping expiry cleanup (expires_at column may not exist)")
            else:
                log(f"  Error cleaning expired signals (HTTPError {e.code}): {e}")
        except Exception as e:
            log(f"  Error cleaning expired signals ({type(e).__name__}): {e}")
    ...
    log("=" * 60)
    log(f"SCAN COMPLETE")
    log(f"  Detectors run:    {len(run_list)}")
    log(f"  Signals detected: {len(all_signals)}")
    log(f"  Duplicates skip:  {_stats['duplicates']}")
    log(f"  Detector errors:  {errors}")
    log(f"  Store errors:     {_stats['store_errors']}")
