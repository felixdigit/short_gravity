...
try:
    ...
    from discord.notify import notify_press_release
...
def fetch_story(story_id: str) -> Optional[Dict]:
    ...
    # Extract original source URL from HTML (BusinessWire, PR Newswire, etc.)
    source_url = None
    for pattern in [
        r'(https://www\.businesswire\.com/news/home/\d+/en/)',
        r'(https://www\.prnewswire\.com/news-releases/[^\s"<>]+)',
    ...
    for i, story_id in enumerate(story_ids):
        ...
        try:
            ...
            if DISCORD_AVAILABLE:
                notify_press_release(title=title, date=str(published_at or ""),
                                     url=story.get("source_url", ""))
        except Exception as e:
            log(f"    ✗ Error: {e}")
            failed += 1
