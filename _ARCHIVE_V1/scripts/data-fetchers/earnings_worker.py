...
the earnings_calls table. Dates only — EPS/revenue estimates are
...
def get_existing_earnings() -> Dict[str, Dict]:
    """Get existing earnings_calls keyed by 'YYYY-Q#'."""
    result = supabase_request(
        "GET",
        f"earnings_calls?company=eq.{COMPANY}&select=id,fiscal_year,fiscal_quarter,call_date,status"
    )
...
def run_worker(dry_run: bool = False):
    """Main worker: fetch Finnhub calendar, upsert into earnings_calls."""
    log("=" * 60)
    ...
    for entry in calendar:
        ...
        if db_row:
            ...
            result = supabase_request(
                "PATCH",
                f"earnings_calls?company=eq.{COMPANY}&fiscal_year=eq.{year}&fiscal_quarter=eq.{quarter}",
                update
            )
            ...
            result = supabase_request("POST", "earnings_calls", record)
            if result is not None:
                created += 1
